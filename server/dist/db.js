...
node_fs_1.default.mkdirSync(node_path_1.default.dirname(node_path_1.default.resolve(file)), { recursive: true });
exports.db = new better_sqlite3_1.default(file);
...
`);
try {
    exports.db.exec("ALTER TABLE fechamentos ADD COLUMN turno TEXT NOT NULL DEFAULT 'ALMOÇO'");
}
catch { /* coluna já existe */ }
try {
    exports.db.exec('ALTER TABLE usuarios ADD COLUMN cargo TEXT');
}
catch { /* coluna já existe */ }
const unidadesOficiais = ['TERRA E MAR', 'RESTAURANTE E PIZZARIA', 'DELIVERY', 'DOCELATTO'];
const garantirUnidades = exports.db.transaction(() => {
    const add = exports.db.prepare('INSERT OR IGNORE INTO unidades (nome, ativo) VALUES (?, 1)');
    unidadesOficiais.forEach((nome) => add.run(nome));
    exports.db.prepare("UPDATE unidades SET ativo = 0 WHERE nome GLOB 'UNIDADE [0-9]*'").run();
});
garantirUnidades();
